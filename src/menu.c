...
void blit_menu(state_t* state) {
    if (state->menu == MENU_INVENTORY) {
    ...
    } else if (state->menu == (MENU_INVENTORY | MENU_CHEST)) {
        sprite_sheet_blit_text_frame(&state->sprite_sheet, &state->renderer, &state->font, 0, 0, 0, 14,
                                     rgb_to_palette(5, 3, 0), rgb_to_palette(3, 2, 1), FLIP_NONE,
                                     string_create("INVENTORY"));

        sprite_sheet_blit_text_frame(&state->sprite_sheet, &state->renderer, &state->font, 152, 0, 6, 14,
                                     rgb_to_palette(5, 3, 0), rgb_to_palette(3, 2, 1), FLIP_NONE,
                                     string_create("CHEST"));
        ...
        pawn_t* chest = list_get(&state->arcade.pawns, state->arcade.pawn->furniture_id);

        for (usize i = 0; i < chest->inventory.length; i++) {
            item_stack_t* item = chest->inventory.items[i];

            item_blit(item->item, state, 118, i * 8 + 16);
            BLIT_NUMBER(item->amount, 116, i * 8 + 16, rgb_to_palette(5, 3, 0), rgb_to_palette(3, 2, 1));
        }
    }
}
