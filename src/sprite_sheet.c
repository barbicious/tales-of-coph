...
void sprite_sheet_blit_sprite(sprite_sheet_t *sprite_sheet, renderer_t *renderer, i32 ox, i32 oy, u32 sx, u32 sy, u32 width, u32 height) {
    for (i32 y = 0; y < height; y++) {
        for (i32 x = 0; x < width; x++) {
            if (SPRITE_SHEET_PIXEL(x, y) == OPAQUE) {
                continue;
            }

            renderer_set_pixel(renderer, x, y, SPRITE_SHEET_PIXEL(x, y) * 4);
        }
    }
}